...
const CardSummaryCheckOut: React.FC<CartSummaryProps> = ({
  selectedItems,
  isDark,
  onCheckout,
  shippingFee,
  ...
  const isService = typeCheckOut === "service";

  const isCheckoutDisabled =
    selectedItems.length === 0 ||
    selectedPaymentMethod == null ||
    (!isService && selectedDelivery == null);
  // subtotal produk
  const subtotal = selectedItems.reduce(
    (sum, item) =>
      sum + (item.priceDiscount || item.price) * (item.quantity || 1),
    0
  );
  ...
  const totalDiscount = selectedItems.reduce(
    (sum, item) =>
      sum +
      (item.price - (item.priceDiscount || item.price)) * (item.quantity || 1),
    0
  );
