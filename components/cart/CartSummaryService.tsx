...
const CartSummaryService: React.FC<CartSummaryProps> = ({
  selectedItems,
  isDark,
  onCheckout,
}) => {
  const totalPrice = selectedItems.reduce(
    (sum, item) =>
      sum +
      (item.service.normalPrice -
        (item.service.normalPrice * item.service.discountPercent) / 100) *
        (item.qty || 1),
    0
  );
...
export default CartSummaryService;
